module Formtastic #:nodoc:
  ...
  class SemanticFormBuilder < ActionView::Helpers::FormBuilder
    ...
    def options_for_label(options)
      options.slice(:label, :required, :as_span).merge!(options.fetch(:label_html, {}))
    end
    ...
    def date_or_datetime_input(method, options)
      ...
      hidden_fields_capture + field_set_and_list_wrapping_for_method(method, options.reverse_merge!(:as_span => false), list_items_capture)
    end
    ...
    def field_set_and_list_wrapping_for_method(method, options, contents)
      ...
      template.content_tag(:fieldset,
        %{<legend>#{self.label(method, options_for_label(options).reverse_merge!(:as_span => true))}</legend>} +
